...
wxLed::wxLed(wxWindow* parent, wxWindowID id, const char* disabledColor,
...
wxLed::~wxLed() { delete this->m_bitmap; }

bool wxLed::Enable(bool enable) {
    bool status = wxWindow::Enable(enable);
    this->SetBitmap(this->m_isEnabled ? this->m_enabledColor
...
void wxLed::SetBitmap(const char* color) {
    if (strncmp(this->m_color, color, 6)) {
        ...
        // sprintf (xpm [0], "%d %d %d 1", WX_LED_WIDTH, WX_LED_HEIGHT,
        // WX_LED_COLORS, WX_LED_XPM_COLS) ;
        sprintf(xpm[0], "%d %d %d 1", WX_LED_WIDTH, WX_LED_HEIGHT,
                WX_LED_COLORS);
        // colors
        ...
        this->SetSize(
            wxSize(this->m_bitmap->GetWidth(), this->m_bitmap->GetHeight()));
        this->m_mutex.Unlock();
        this->Refresh();
