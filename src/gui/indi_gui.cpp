...
public:
    IndiStatus(wxWindow *parent, wxWindowID id, IPState state)
        : wxLed(parent, id) {
        SetState(state);
        Enable();
    }
...
wxBEGIN_EVENT_TABLE(IndiGui, wxDialog) EVT_CLOSE(IndiGui::OnQuit)
    EVT_THREAD(INDIGUI_THREAD_NEWDEVICE_EVENT, IndiGui::OnNewDeviceFromThread)
        EVT_THREAD(INDIGUI_THREAD_NEWPROPERTY_EVENT,
                   IndiGui::OnNewPropertyFromThread)
            EVT_THREAD(INDIGUI_THREAD_NEWNUMBER_EVENT,
                       IndiGui::OnNewNumberFromThread)
                EVT_THREAD(INDIGUI_THREAD_NEWTEXT_EVENT,
                           IndiGui::OnNewTextFromThread)
                    EVT_THREAD(INDIGUI_THREAD_NEWSWITCH_EVENT,
                               IndiGui::OnNewSwitchFromThread)
                        EVT_THREAD(INDIGUI_THREAD_NEWMESSAGE_EVENT,
                                   IndiGui::OnNewMessageFromThread)
                            EVT_THREAD(INDIGUI_THREAD_REMOVEPROPERTY_EVENT,
                                       IndiGui::OnRemovePropertyFromThread)
                                wxEND_EVENT_TABLE()
    ...
    wxThreadEvent *event =
        new wxThreadEvent(wxEVT_THREAD, INDIGUI_THREAD_NEWDEVICE_EVENT);
...
void IndiGui::newProperty(INDI::Property *property) {
    wxThreadEvent *event =
        new wxThreadEvent(wxEVT_THREAD, INDIGUI_THREAD_NEWPROPERTY_EVENT);
...
void IndiGui::newNumber(INumberVectorProperty *nvp) {
    wxThreadEvent *event =
        new wxThreadEvent(wxEVT_THREAD, INDIGUI_THREAD_NEWNUMBER_EVENT);
...
void IndiGui::newSwitch(ISwitchVectorProperty *svp) {
    wxThreadEvent *event =
        new wxThreadEvent(wxEVT_THREAD, INDIGUI_THREAD_NEWSWITCH_EVENT);
...
void IndiGui::newText(ITextVectorProperty *tvp) {
    wxThreadEvent *event =
        new wxThreadEvent(wxEVT_THREAD, INDIGUI_THREAD_NEWTEXT_EVENT);
...
void IndiGui::newMessage(INDI::BaseDevice *dp, int messageID) {
    wxThreadEvent *event =
        new wxThreadEvent(wxEVT_THREAD, INDIGUI_THREAD_NEWMESSAGE_EVENT);
...
void IndiGui::removeProperty(INDI::Property *property) {
    if (property) {
        ...
        wxThreadEvent *event = new wxThreadEvent(
            wxEVT_THREAD, INDIGUI_THREAD_REMOVEPROPERTY_EVENT);
...
void IndiGui::OnNewDeviceFromThread(wxThreadEvent &event) {
    ...
    indiDev->page = new wxNotebook(panel, wxID_ANY, wxDefaultPosition,
...
void IndiGui::OnNewPropertyFromThread(wxThreadEvent &event) {
    ...
    // printf("newproperty from thread %s %s
    ...
    gbs->Add(indiProp->state, POS(next_free_row, 0), SPAN(1, 1),
             wxALIGN_LEFT | wxALL);
    gbs->Add(indiProp->name, POS(next_free_row, 1), SPAN(1, 1),
             wxALIGN_LEFT | wxALL);
    gbs->Add(indiProp->panel, POS(next_free_row, 2), SPAN(1, 1),
...
void IndiGui::CreateSwitchCombobox(ISwitchVectorProperty *svp,
                                   IndiProp *indiProp) {
    ...
    for (int i = 0; i < svp->nsp; i++) {
        if (svp->sp[i].s == ISS_ON)
            idx = i;
        indiProp->ctrl[wxString::FromAscii(svp->sp[i].name)] =
            (void *)(intptr_t)i;
    ...
    wxChoice *combo = new wxChoice(p, wxID_ANY, wxDefaultPosition,
                                   wxDefaultSize, svp->nsp, choices);
...
void IndiGui::CreateSwitchCheckbox(ISwitchVectorProperty *svp,
                                   IndiProp *indiProp) {
    wxPanel *p = indiProp->panel;
    wxGridBagSizer *gbs = indiProp->gbs;
    for (int pos = 0; pos < svp->nsp; pos++) {
        ...
        gbs->Add(button, POS(pos / 4, pos % 4), SPAN(1, 1),
                 wxALIGN_LEFT | wxALL);
    }
}

void IndiGui::CreateSwitchButton(ISwitchVectorProperty *svp,
                                 IndiProp *indiProp) {
    wxPanel *p = indiProp->panel;
    wxGridBagSizer *gbs = indiProp->gbs;
    for (int pos = 0; pos < svp->nsp; pos++) {
...
void IndiGui::CreateTextWidget(INDI::Property *property, IndiProp *indiProp) {
    ...
    for (pos = 0; pos < tvp->ntp; pos++) {
        gbs->Add(new wxStaticText(p, wxID_ANY,
                                  wxString::FromAscii(tvp->tp[pos].label)),
                 POS(pos, 0), SPAN(1, 1), wxALIGN_LEFT | wxALL);

        wxStaticText *value = new wxStaticText(
            p, wxID_ANY, wxString::FromAscii(tvp->tp[pos].text));
...
void IndiGui::CreateNumberWidget(INDI::Property *property, IndiProp *indiProp) {
    ...
    for (pos = 0; pos < nvp->nnp; pos++) {
        gbs->Add(new wxStaticText(p, wxID_ANY,
                                  wxString::FromAscii(nvp->np[pos].label)),
                 POS(pos, 0), SPAN(1, 1), wxALIGN_LEFT | wxALL);

        wxStaticText *value = new wxStaticText(
            p, wxID_ANY, wxString::Format(_T("%f"), nvp->np[pos].value));
    ...
    switch (indiProp->property->getType()) {
        case INDI_TEXT: {
            ...
            for (int i = 0; i < tvp->ntp; i++) {
                if (tvp->p != IP_RO) {
                    wxTextCtrl *entry =
                        (wxTextCtrl *)(indiProp->entry[wxString::FromAscii(
                            tvp->tp[i].name)]);
                    sprintf(tvp->tp[i].text, "%s",
                            entry->GetLineText(0).mb_str().data());
                }
            }
            sendNewText(tvp);
            break;
...
void IndiGui::ShowIndiGui(IndiGui **ret, const wxString &host, long port,
                          bool allow_connect_disconnect_, bool modal) {
    IndiGui *gui = new IndiGui();
    gui->allow_connect_disconnect = allow_connect_disconnect_;
    gui->ConnectServer(host, port);

    {
        wxProgressDialog dlg(_("INDI"), _("Loading INDI properties..."), 0,
                             nullptr, wxPD_APP_MODAL | wxPD_CAN_ABORT);

        enum { IDLE_TIME_MS = 500 };

        unsigned int i = 0;
        while (wxGetUTCTimeMillis().GetValue() - gui->m_lastUpdate.GetValue() <
               IDLE_TIME_MS) {
            wxSafeYield();
            wxMilliSleep(10);
            if (dlg.WasCancelled()) {
...
IndiGui::IndiGui()
    : wxDialog(wxGetApp().GetTopWindow(), wxID_ANY, _("INDI Options"),
               wxDefaultPosition, wxSize(640, 400),
               wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER),
      m_deleted(false),
      m_holder(nullptr) {
    panel = new wxPanel(this, wxID_ANY, wxDefaultPosition, wxDefaultSize,
                        wxBORDER_DOUBLE | wxTAB_TRAVERSAL);
    sizer = new wxBoxSizer(wxVERTICAL);
    panel->SetSizer(sizer);
    parent_notebook = new wxNotebook(panel, wxID_ANY, wxDefaultPosition,
                                     wxDefaultSize, wxNB_TOP);
...
IndiGui::~IndiGui() {
    ...
    for (auto itdev = devlist.begin(); itdev != devlist.end(); ++itdev) {
        ...
        if (indiDev) {
            for (auto itprop = indiDev->properties.begin();
                 itprop != indiDev->properties.end(); ++itprop) {
