...
double Spline::Eval(double x) const {
    return gsl_spline_eval(static_cast<gsl_spline*>(spline), x,
                           static_cast<gsl_interp_accel*>(accel));
}
...
struct LFit {
    ...
    void reset() { avx = avy = varx = covxy = vary = n = 0.; }
    // y = a + b x
...
static double PolarAlignError(const GuideSession& session) {
    ...
    return 3.8197 * fabs(session.drift_dec) * session.pixelScale /
           cos(session.declination);
}
...
struct Line {
    double a, b;
    Line(const LFit& lf) { lf.result(&a, &b); }
...
bool GARun::CanAnalyze(const GuideSession& session, size_t begin, size_t end) {
    ...
    enum {
        MIN_ENTRIES = 12
    };  // need at least 12 for FFT output spline (N / 2 - 1 >= 5)
...
void GARun::Analyze(const GuideSession& session, size_t begin, size_t end,
...

    {
        ...
        for (unsigned int i = 0; i < n; i++, x += dt) {
            if (x > t[n - 1])
                x = t[n - 1];  // rounding error can put the last point over the
                               // boundary
            // Hamming window
...

    {
        ...
        double scale =
            4. /
            (double)
                n;  // http://www.stat.ucla.edu/~frederic/221/W17/221ch4a.pdf
        fftymax = 0.;
...
struct GraphPos {
    int x0, y0, x1, y1;  // screen coords of upper left and lower right corners
                         // of graph (accomodates padding)
...
struct FFTPos : public GraphPos {
    ...
    void VZoom(double d) { scy *= d; }
    double P(int x) const { return p0 * exp(((double)x - xofs) * scx); }
    int X(double p) { return (int)(floor(log(p / p0) / scx + xofs)); }
    int Eval(int x) {
        double p = P(x);
    ...
    int StartX() const { return std::max(x0, (int)ceil(xofs)); }
    int EndX() const { return std::min(x1, (int)floor(xofs) + xw); }
};
...
struct DriftPos : public GraphPos {
    ...
    double T(int x) { return ((double)x - xofs) * scx; }
    int X(double t) { return (int)(t / scx + xofs); }
...
wxBEGIN_EVENT_TABLE(AnalysisWin, AnalyzeFrameBase)
    EVT_MOUSEWHEEL(AnalysisWin::OnMouseWheel) wxEND_EVENT_TABLE()

        AnalysisWin::AnalysisWin(LogViewFrame* parent)
    : AnalyzeFrameBase(parent), m_cursor(-1) {
    m_graph->SetBackgroundStyle(wxBG_STYLE_PAINT);
    LoadGeometry(this, "/geometry.awin");

    m_graph->Connect(wxEVT_MOUSE_CAPTURE_LOST,
                     wxMouseCaptureLostEventHandler(AnalysisWin::OnCaptureLost),
                     nullptr, this);
}
...
static void GetGABounds(const GuideSession& session, size_t pos, size_t* begin,
                        size_t* end) {
    const auto& entries = session.entries;
...
void AnalysisWin::AnalyzeAll(const GuideSession& session,
    ...
    s_drpos.Init(m_graph->GetSize(), m_garun);
    s_fftpos.Init(m_graph->GetSize(), m_garun);
    SetTitle(undo_ra_corrections ? _("Analysis ** RA Corrections Removed **")
                                 : _("Analysis"));
}

void AnalysisWin::OnCheck(wxCommandEvent& event) { m_graph->Refresh(); }
...
void AnalysisWin::OnBtnLeftDown(wxMouseEvent& event) {
    wxToggleButton* btn =
        reinterpret_cast<wxToggleButton*>(event.GetEventObject());
    if (!btn->GetValue()) {
        event.Skip();
        return;
    }
...
static void HZoomOut(AnalysisWin* aw, int center) { HZoom(aw, 1.1, center); }
...
void AnalysisWin::OnMove(wxMouseEvent& event) {
    if (s_drag.dragging) {
        ...
        if (dx != 0) {
            double xofs =
                s_drag.dragStartXofs + (double)(pos.x - s_drag.dragStart.x);
            if (m_toggleFFT->GetValue()) {
                s_fftpos.xofs = xofs;
                m_cursor = -1;
            } else {
    ...
    if (m_toggleFFT->GetValue()) {
        ...
        m_statusBar->SetStatusText(wxString::Format(
            "Period: %.1fs  Amplitude: %.1f\" (%.2fpx)  P-P: %.1f\" (%.2fpx)  "
            "RMS: %.1f\" (%.2fpx)",
        ...
        if (m_garun.len >= 2 && t >= m_garun.t[0] &&
            t <= m_garun.t[m_garun.len - 1]) {
            int y = event.GetPosition().y;
            double yval = s_drpos.RaOrDec(y);
            s = wxString::Format(
                "Time: %-.1fs  %s    Y: %.2f\" (%.2fpx)", t,
                (m_garun.starts + wxTimeSpan(0, 0, t)).Format("%H:%M:%S"),
...
static double IncrP(double p) { return pow(10.0, floor(log10(p))); }
...
static void PaintFFT(AnalysisWin* aw, const GARun& ga, wxDC& dc) {
...

    {
        ...
        if (arcsecs)  // arc-seconds
            vsc /= ga.pixscale;
        double v =
            (double)aw->m_graph->GetSize().GetHeight() * (0.5 / 6.0) / vsc;
        double m = pow(10, ceil(log10(v)));
        double t;
        if (v < (t = .25 * m))
            v = t;
...
void AnalysisWin::OnHMinus(wxCommandEvent& event) { HZoomOut(this, -1); }

void AnalysisWin::OnHPlus(wxCommandEvent& event) { HZoomIn(this, -1); }
