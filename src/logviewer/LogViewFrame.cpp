...
wxBEGIN_EVENT_TABLE(LogViewFrame, LogViewFrameBase)
    EVT_MENU(wxID_OPEN, LogViewFrame::OnFileOpen)
        EVT_MENU(wxID_SETTINGS, LogViewFrame::OnFileSettings)
            EVT_MENU(wxID_EXIT,
                     LogViewFrame::OnFileExit) EVT_MENU(wxID_HELP,
                                                        LogViewFrame::OnHelp)
                EVT_MENU(wxID_ABOUT, LogViewFrame::OnHelpAbout)
                    EVT_MENU_RANGE(ID_INCLUDE_ALL, ID_EXCLUDE_SETTLE,
                                   LogViewFrame::OnMenuInclude)
                        EVT_MENU(ID_ANALYZE_GA, LogViewFrame::OnMenuAnalyzeGA)
                            EVT_MENU_RANGE(ID_ANALYZE_ALL, ID_ANALYZE_ALL_NORA,
                                           LogViewFrame::OnMenuAnalyzeAll)
...
void SaveGeometry(const wxFrame* win, const wxString& key) {
    Config->Write(key,
                  wxString::Format("%d;%d;%d;%d;%d", win->IsMaximized() ? 1 : 0,
...
LogViewFrame::LogViewFrame()
    ...
    m_graph->Connect(
        ...
        wxMouseCaptureLostEventHandler(LogViewFrame::OnCaptureLost), NULL,
        this);
    ...
    s_settings.excludeByServer =
        Config->ReadBool("/settle/excludeByServer", true);
    s_settings.excludeParametric =
        Config->ReadBool("/settle/excludeParametric", false);
    ...
    s_settings.raColor = wxColor(Config->Read(
        "/color/ra", wxColor(100, 100, 255).GetAsString(wxC2S_HTML_SYNTAX)));
    s_settings.decColor = wxColor(
        Config->Read("/color/dec", wxRED->GetAsString(wxC2S_HTML_SYNTAX)));
...
inline static void IncludeRange(GuideSession::EntryVec& entries, bool include,
                                unsigned int i = 0,
                                unsigned int i1 = (unsigned int)-1) {
    for (auto it = entries.begin() + i; i < i1 && it != entries.end();
         ++it, ++i)
        it->included = include;
}
...
static void ExcludeSettlingByAPI(GuideSession* session) {
    ...
    for (auto it = infos.begin(); it != infos.end(); ++it) {
        if (settling) {
            if (it->info.find("Settling complete") != wxString::npos ||
                it->info.find("Settling fail") != wxString::npos) {
                settling = false;
...
static void ExcludeSettlingByDistance(GuideSession* session,
                                      const SettleParams& params) {
    auto& infos = session->infos;
    auto& entries = session->entries;
...
void LogViewFrame::OpenLog(const wxString& filename) {
    ...
    // must do this before calling parser since parse will Yield and call
    // OnPaint
    m_sessionIdx = -1;
    m_session = 0;
    m_sessionInfo->Clear();
    ...
    if (!s_log.phd_version.empty())
        SetTitle(wxString::Format(APP_NAME " - %s - LGuider2 %s",
                                  fn.GetFullName(), s_log.phd_version.c_str()));

    // load the grid
    m_sessions->BeginBatch();
    int row = 0;
    for (auto it = s_log.sections.begin(); it != s_log.sections.end();
         ++it, ++row) {
...
void LogViewFrame::OnFileExit(wxCommandEvent& event) { Close(); }

void LogViewFrame::OnFileOpen(wxCommandEvent& event) {
    wxFileDialog openFileDialog(
        this, _("Open LGuider2 Guide Log"),
        Config->Read("/FileOpenDir", wxEmptyString), wxEmptyString,
    ...
    // save the location
    Config->Write("/FileOpenDir",
                  wxFileName(openFileDialog.GetPath()).GetPath());
...
void LogViewFrame::OnFileSettings(wxCommandEvent& event) {
    ...
    dlg.m_settlePixels->SetValidator(
        wxFloatingPointValidator<double>(2, &pixels, 0));
    double seconds = s_settings.settle.seconds;
    dlg.m_settleSeconds->SetValidator(
        wxFloatingPointValidator<double>(1, &seconds, 0));
    ...
    Config->Write("/color/ra",
                  s_settings.raColor.GetAsString(wxC2S_HTML_SYNTAX));
    Config->Write("/color/dec",
                  s_settings.decColor.GetAsString(wxC2S_HTML_SYNTAX));
}
...
void LogViewFrame::OnRightUp(wxMouseEvent& event) {
    ...
    wxMenuItem* mi1 =
        menu->Append(ID_ANALYZE_ALL, _("Analyze selected frames"));
    wxMenuItem* mi2 =
        menu->Append(ID_ANALYZE_ALL_NORA, _("Analyze selected, raw RA"));
...
static void InitStats(wxGrid* stats, wxHtmlWindow* stats2,
                      const GuideSession* session) {
    stats->BeginBatch();
    stats->SetCellValue(0, 0,
                        wxString::Format("% .2f\" (%.2f px)",
                                         session->pixelScale * session->rms_ra,
                                         session->rms_ra));
    stats->SetCellValue(1, 0,
                        wxString::Format("% .2f\" (%.2f px)",
                                         session->pixelScale * session->rms_dec,
                                         session->rms_dec));
    double tot = sqrt(session->rms_ra * session->rms_ra +
                      session->rms_dec * session->rms_dec);
    stats->SetCellValue(
        2, 0,
        wxString::Format("% .2f\" (%.2f px)", session->pixelScale * tot, tot));
    stats->SetCellValue(0, 1,
                        wxString::Format("% .2f\" (% .2f px)",
                                         session->pixelScale * session->peak_ra,
                                         session->peak_ra));
    stats->SetCellValue(
        1, 1,
        wxString::Format("% .2f\" (% .2f px)",
                         session->pixelScale * session->peak_dec,
                         session->peak_dec));
    stats->SetCellValue(
        2, 1, wxString::Format(" Elong.: %.f%%", session->elongation * 100.));
    stats->AutoSize();
    stats->EndBatch();

    std::ostringstream os;
    os << "<span style='font-family:monospace;font-size:8;'><table "
          "cellspacing=1 cellpadding=1 border=0>"
          "<tr><td>RA Drift</td><td>"
       << FormatNum(session->drift_ra * session->pixelScale) << "\"/min, "
       << FormatNum(session->drift_ra) << " px/min</td></tr>"
       << "<tr><td>Dec Drift</td><td>"
       << FormatNum(session->drift_dec * session->pixelScale) << "\"/min, "
       << FormatNum(session->drift_dec) << " px/min</td></tr>"
       << "<tr><td>Polar Alignment Error  </td><td>" << std::fixed
       << std::setprecision(1) << session->paerr << "'</td></tr>"
       << "</table></span>";
    stats2->SetPage(os.str());
}

static void UpdateStats(wxGrid* stats, wxHtmlWindow* stats2,
...
void LogViewFrame::OnHelpAbout(wxCommandEvent& event) {
    ...
    aboutInfo.SetDescription(
        _("A tool for visualizing LGuider2 guide log data"));
    aboutInfo.SetCopyright(
        "(C) 2018-2020 Andy Galasso <andy.galasso@gmail.com>");
...
HelpDialog::HelpDialog(wxWindow* parent) : HelpDialogBase(parent) {
    ...
    s << "<html>" << "<h3>Opening a guide log</h3>"
      ...
      << "<h3>Viewing calibration</h3>" << "Drag with the mouse to pan" << BR
      << "Use the mouse wheel or the lower +/- buttons to zoom in and out" << BR
      << "<h3>Viewing guiding data</h3>"
      << "Drag left or right with the mouse to pan, or use the horizontal "
         "scrollbar"
      << BR
      << "The mouse wheel or the lower +/- buttons will zoom the horizontal "
         "scale"
      << BR
      << "Drag up or down with the mouse or use the +/- buttons on the right "
         "to change the vertical scale"
      << BR << "<h3>Statistics</h3>"
      << "To exclude a range of points from the statistics, hold down CTRL and "
         "drag the mouse,"
      << BR
      << "To include a range of points from the statistics, hold down Shift "
         "and drag the mouse,"
      << BR
      << "CTRL-Click on an excluded range to un-exclude the range of points."
      << BR << "Right-click on the graph for some more options." << BR
      << "</html>";
...
HelpDialog::~HelpDialog() { s_help = 0; }
...
void LogViewFrame::InitGraph() {
    ...
    for (auto it = m_session->entries.begin(); it != m_session->entries.end();
         ++it) {
        const auto& e = *it;
...
void LogViewFrame::InitCalDisplay() {
    ...
    for (auto it = m_calibration->entries.begin();
         it != m_calibration->entries.end(); ++it) {
        const auto& p = *it;
        if (p.direction == WEST) {
            if (disp.firstWest == -1)
                disp.firstWest = i;
    ...
    if (maxv == 0.0)
        maxv = 25.0;
    int size =
        wxMin(m_graph->GetSize().GetWidth(), m_graph->GetSize().GetHeight());
...
inline static wxLongLong_t now() { return ::wxGetUTCTimeMillis().GetValue(); }
...
void LogViewFrame::OnLeftUp(wxMouseEvent& event) {
    ...
    if (s_drag.m_dragMode == DRAG_PAN) {
        ...
        if (dt > 0) {
            s_drag.m_rate.x =
                (double)(event.GetPosition().x - s_drag.m_mousePos[0].x) /
                (double)dt;
            s_drag.m_rate.y =
                (double)(event.GetPosition().y - s_drag.m_mousePos[0].y) /
                (double)dt;
            s_drag.decel = DECEL;
            if (fabs(s_drag.m_rate.x) > DECEL ||
                fabs(s_drag.m_rate.y) > DECEL) {
    ...
    if (m_session) {
        ...
        if (!s_drag.m_dragging) {
            ...
            if (i >= 0 && i < (int)entries.size()) {
                ...
                wxDateTime t(
                    m_session->starts +
                    wxTimeSpan(0, 0, 0, (wxLongLong)(ent.dt * 1000.0)));
                m_rowInfo->SetValue(wxString::Format(
                    "%s Frame %d t=%.2f (x,y)=(%.2f,%.2f) (RA,Dec)=(%.2f,%.2f) "
                    "guide (%.2f,%.2f) corr (%d,%d) m=%d SNR=%.1f%s %s",
                    t.FormatISOCombined(' '), ent.frame, ent.dt, ent.dx, ent.dy,
                    ent.raraw, ent.decraw, ent.raguide, ent.decguide, ent.radur,
                    ent.decdur, ent.mass, ent.snr, ent.err == 1 ? " SAT" : "",
                    ent.info));
            }
        ...
        if (s_drag.m_dragMode == DRAG_PAN) {
            ...
            if (dy != 0) {
                if (vpan) {
                    ...
                    double vscale =
                        vscale_locked()
                            ? get_vscale_setting(m_session->pixelScale)
                            : ginfo.vscale;
...
void LogViewFrame::OnScroll(wxScrollEvent& event) {
    ...
    if (t == wxEVT_SCROLL_THUMBRELEASE || t == wxEVT_SCROLL_CHANGED) {
        event.Skip();
        return;
    }
...
void LogViewFrame::UpdateScrollbar() {
    ...
    int end1 = (int)(ginfo.hscale * (double)entries.size()) -
               ginfo.xofs;  // end of curve, screen coords
...
static void PaintCalibration(wxDC& dc, const Calibration* cal,
                             wxWindow* graph) {
    ...
    for (auto p = cal->entries.rbegin(); p != cal->entries.rend(); ++p) {
        ...
        dc.DrawText(wxString::Format("%c%d", ch, p->step),
                    x0 + (int)(p->dx * scale) + 8,
                    y0 - (int)(p->dy * scale) + 8);
    }

    if (cal->display.firstWest != -1 &&
        cal->display.lastWest != cal->display.firstWest) {
    ...
    if (cal->display.firstNorth != -1 &&
        cal->display.lastNorth != cal->display.firstNorth) {
...
void LogViewFrame::OnPaintGraph(wxPaintEvent& event) {
    ...
    const GuideSession::EntryVec& entries = m_session->entries;
    const GraphInfo& ginfo = m_session->m_ginfo;
    double const vscale = vscale_locked()
                              ? get_vscale_setting(m_session->pixelScale)
                              : ginfo.vscale;
    ...
    if (m_grid->IsChecked()) {
        ...
        if (iv > 0) {
            ...
            dy = -v;
            for (int y = y0 + iv; y < m_graph->GetSize().GetHeight();
                 y += iv, dy -= v) {
                dc.DrawLine(0, y, fullw, y);
                dc.DrawText(wxString::Format(format, dy), 3, y + 2);
            }
        ...
        if (i1 > i0) {
            ...
            int secs =
                (int)(ceil(80.0 / (r * 60.0)) * 60.0);  // seconds per tick
            wxDateTime ti0(m_session->starts +
                           wxTimeSpan(0, 0, 0, (wxLongLong)(dt0 * 1000.0)));
            wxDateTime t0(ti0);
            ...
            wxDateTime t1(m_session->starts +
                          wxTimeSpan(0, 0, 0, (wxLongLong)(dt1 * 1000.0)));
            double tend =
                (double)(t1 - ti0).GetMilliseconds().GetValue() / 1000.0;
            ...
            for (; t < tend; t += secs) {
                ...
                wxDateTime wxt(ti0 +
                               wxTimeSpan(0, 0, 0, (wxLongLong)(t * 1000.0)));
    ...
    if (m_limits->IsChecked()) {
        if (m_ra->IsChecked()) {
            if (m_session->mount.xlim.maxDur > 0.0) {
                ...
                int y = (int)(m_session->mount.xlim.maxDur *
                              m_session->mount.xRate / 1000.0 * vscale);
        ...
        if (m_dec->IsChecked()) {
            if (m_session->mount.ylim.maxDur > 0.0) {
                ...
                int y = (int)(m_session->mount.ylim.maxDur *
                              m_session->mount.yRate / 1000.0 * vscale);
    ...
    if (m_corrections->IsChecked() && m_ra->IsChecked() && cwid >= 1) {
        ...
        dc.DrawText(lblE, fullw - szE.GetWidth() - 4,
                    m_graph->GetSize().GetHeight() - 2 * szE.GetHeight() - 2);
    ...
    if (m_corrections->IsChecked() && m_dec->IsChecked() && cwid >= 1) {
        ...
        dc.DrawText(lblN, fullw - szN.GetWidth() - 4,
                    0 /*topEdge*/ + szN.GetHeight() + 2);
        dc.SetTextForeground(prev);
    ...
    if (m_mass->IsChecked()) {
        double massscale;
        if (ginfo.max_mass > 0)
            massscale = (double)(m_graph->GetSize().GetHeight() / 2 - 10) /
                        (double)ginfo.max_mass;
        else
            massscale = 1.0;
    ...
    if (m_snr->IsChecked()) {
        ...
        if (ginfo.max_snr > 0.0)
            snrscale = (double)(m_graph->GetSize().GetHeight() / 2 - 10) /
                       ginfo.max_snr;
        else
            snrscale = 1.0;
    ...
    if (i1 >= i0) {
        ...
        for (unsigned int i = i0 + 1; i <= i1; i++) {
            ...
            if (included && !prev_included) {
                ...
                if (gc) {
                    int x0 =
                        (int)((double)(e0 - 0.25) * ginfo.hscale) - ginfo.xofs;
                    int x1 = (int)((double)(i - 1.0 + 0.25) * ginfo.hscale) -
                             ginfo.xofs;
                    gc->DrawRectangle(x0, 0, x1 - x0 + 1,
                                      m_graph->GetSize().GetHeight());
                }
    ...
    if (s_drag.m_dragging &&
        ...
        if (gc) {
            ...
            gc->DrawRectangle(rect.x, 0, rect.width,
                              m_graph->GetSize().GetHeight());
            delete gc;
        }
    }
    ...
    if (m_scatter->IsChecked()) {
        ...
        if (!s_scatter.valid) {
            if (!s_scatter.bitmap ||
                s_scatter.bitmap->GetSize().GetWidth() != h) {
            ...
            double scale = vscale * h * 0.5 /
                           (double)(m_graph->GetSize().GetHeight() / 2 - 10);

            for (auto it = entries.begin(); it != entries.end(); ++it) {
                if (it->included) {
                    int x = (int)((double)(radec ? it->raraw : it->dx) * scale);
                    int y =
                        (int)((double)(radec ? it->decraw : it->dy) * scale);
            ...
            // draw an ellipse showing the elongation
            {
                ...
                gc->Translate(h / 2 + m_session->avg_ra * scale,
                              h / 2 - m_session->avg_dec * scale);
    ...
    if (m_vlock->GetValue()) {
        ...
        for (auto it = s_log.sections.begin(); it != s_log.sections.end();
            ...
            if (it->type == GUIDING_SECTION) {
                GuideSession& session =
                    static_cast<GuideSession&>(s_log.sessions[it->idx]);
...
void LogViewFrame::OnDevice(wxCommandEvent& event) { m_graph->Refresh(); }
...
void LogViewFrame::OnDecChecked(wxCommandEvent& event) { m_graph->Refresh(); }

void LogViewFrame::OnRAChecked(wxCommandEvent& event) { m_graph->Refresh(); }

void LogViewFrame::OnIconize(wxIconizeEvent& event) { event.Skip(); }

void LogViewFrame::OnActivate(wxActivateEvent& event) { event.Skip(); }
...
void LogViewFrame::OnClose(wxCloseEvent& event) {
    ...
    Config->Write("/geometry.splitter1",
                  wxString::Format("%d", m_splitter1->GetSashPosition()));
    Config->Write("/geometry.splitter2",
                  wxString::Format("%d", m_splitter2->GetSashPosition()));
    event.Skip();
}
        ...
        case WXK_CONTROL_C:
            if (wxClipboard::Get()->Open()) {
                wxClipboard::Get()->SetData(
                    new wxTextDataObject(StatsText(m_stats)));
