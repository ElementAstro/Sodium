...
static const wxCmdLineEntryDesc cmdLineDesc[] = {
    ...
    {wxCMD_LINE_OPTION, "i", "instanceNumber",
     "sets the PHD2 instance number (default = 1)", wxCMD_LINE_VAL_NUMBER,
     wxCMD_LINE_PARAM_OPTIONAL},
    {wxCMD_LINE_OPTION, "l", "load", "load settings from file and exit",
     wxCMD_LINE_VAL_STRING, wxCMD_LINE_PARAM_OPTIONAL},
    {wxCMD_LINE_SWITCH, "R", "Reset",
     "Reset all PHD2 settings to default values"},
        ...
        (major == 10 && minor >= 9))  // Mavericks or later -- deal with App Nap
    {
             ...
             out[0].Contains("0")))  // it's not there or disabled
        {
            ...
            wxMessageBox(
                ...
                "relaunch PHD to finish disabling App Nap.");
        }
    }
#undef APPKEY
#endif
...
void PhdApp::RestartApp() {
    ...
    wchar_t **targv = new wchar_t *[argc + 2];
    targv[0] = wxStrdup(argv[0].wc_str());
...
void setup_logger() {
    try {
        ...
        spdlog::flush_on(spdlog::level::debug);    // 刷新级别
...
bool PhdApp::OnInit() {
    ...
    if (m_instanceChecker->IsAnotherRunning()) {
        wxLogError(
            wxString::Format(_("PHD2 instance %ld is already running. Use the "
                               "-i INSTANCE_NUM command-line option to start a "
                               "different instance."),
                             m_instanceNumber));
...
#if defined(__WINDOWS__)
    // use per-thread locales on windows to that INDI can set the locale in its
    ...
    Debug.Write(wxString::Format("locale: initialized with lang id %d (r=%d)\n",
                                 langid, ok));
    if (!m_locale.AddCatalog(SODIUM_MESSAGES_CATALOG)) {
...
wxString PhdApp::UserAgent() const {
    return _T("phd2/") FULLVER _T(" (") SODIUM_OSNAME _T(")");
}
