...
// Idle means controller is not in transitional states of
// starting/calibrating/stopping/settling
bool PhdController::IsIdle() { return ctrl.state == STATE_IDLE; }
...
static wxString ReentrancyError(const char *op) {
    return wxString::Format("Cannot initiate %s while %s is in progress", op,
                            ctrl.settleOp == OP_DITHER ? "dither" : "guide");
}

bool PhdController::Guide(unsigned int options, const SettleParams &settle,
...
bool PhdController::Dither(double pixels, bool forceRaOnly,
                           const SettleParams &settle, wxString *errMsg)

{
    ...
    if (pMount && !pMount->IsStepGuider() && !raOnly) {
        // Check to see if we need to force raOnly or temporarily adjust dec
        // guide mode
        ...
        if (dgm != DEC_AUTO) {
            if ((dgm == DEC_NORTH || dgm == DEC_SOUTH) &&
                ...
                // Temporarily override dec guide mode because user has not set
                // ra-only
                ...
                Debug.Write(wxString::Format(
                    "PhdController: forcing dither RA-only since Dec guide "
                    "mode is %s\n",
...
static void do_notify(void) {
    if (ctrl.succeeded) {
        ...
        EvtServer.NotifySettleDone(wxEmptyString, ctrl.settleFrameCount,
                                   ctrl.droppedFrameCount);
        GuideLog.NotifySettlingStateChange("Settling complete");
    } else {
        Debug.AddLine(wxString::Format("PhdController complete: fail: %s",
                                       ctrl.errorMsg));
        EvtServer.NotifySettleDone(ctrl.errorMsg, ctrl.settleFrameCount,
                                   ctrl.droppedFrameCount);
...
static bool start_capturing(void) {
    ...
    pFrame->pGuider
        ->ForceFullFrame();  // we need a full frame to auto-select a star
...
static bool IsAoBumpInProgress() {
    return pMount && pMount->IsStepGuider() &&
           static_cast<StepGuider *>(pMount)->IsBumpInProgress();
}
    ...
    while (!done) {
        switch (ctrl.state) {
            ...
            case STATE_SETUP:
                ...
                ctrl.overrideDecGuideMode =
                    false;  // guide stop/start with no dithering
            ...
            case STATE_ATTEMPT_START: {
                ...
                if (!CanGuide(&err)) {
                    ...
                    // if auto-exposure is enabled, reset to max exposure
                    // duration and wait for the next camera frame
            ...
            case STATE_SELECT_STAR: {
                ...
                if (error) {
                    Debug.Write(wxString::Format(
                        "auto find star failed, attempts remaining = %d\n",
            ...
            case STATE_WAIT_SELECTED:
                if (pFrame->pGuider->GetState() == STATE_SELECTED) {
                    ...
                    Debug.Write(wxString::Format(
                        "waiting for star selected, attempts remaining = %d\n",
                ...
                if ((pMount && !pMount->IsCalibrated()) ||
                    ...
                    if (!start_guiding()) {
                        if (ctrl.useStickyLock && ctrl.haveSaveSticky)
                            pFrame->pGuider->SetLockPosIsSticky(
                                ctrl.saveSticky);
            ...
            case STATE_SETTLE_BEGIN:
                ...
                if (ctrl.overrideDecGuideMode) {
                    Debug.Write(
                        wxString::Format("PhdController: setting Dec guide "
                                         "mode to %s for dither settle\n",
            ...
            case STATE_SETTLE_WAIT: {
                ...
                bool inRange =
                    lockedOnStar && currentError <= ctrl.settle.tolerancePx;
                ...
                Debug.Write(wxString::Format(
                    "PhdController: settling, locked = %d, distance = %.2f "
                    "(%.2f) aobump = %d frame = %d / %d\n",
                    lockedOnStar, currentError, ctrl.settle.tolerancePx,
                    aoBumpInProgress, ctrl.settleFrameCount,
                    ctrl.settle.frames));
                ...
                if (inRange) {
                    ...
                    } else if (((timeInRange = ctrl.settleInRange->Time()) /
                                1000) >= ctrl.settle.settleTimeSec &&
                               !aoBumpInProgress) {
                        ctrl.succeeded = true;
                        SETSTATE(STATE_FINISH);
                        break;
                ...
                if ((ctrl.settleTimeout->Time() / 1000) >=
                    ctrl.settle.timeoutSec) {
                    do_fail(_T("timed-out waiting for guider to settle"));
                    break;
                }
                EvtServer.NotifySettling(
                    currentError, (double)timeInRange / 1000.,
                    ctrl.settle.settleTimeSec, lockedOnStar);
            ...
            case STATE_FINISH:
                if (ctrl.overrideDecGuideMode) {
                    Debug.Write(wxString::Format(
                        "PhdController: restore Dec guide mode to %s after "
                        "dither\n",
                        Scope::DecGuideModeStr(ctrl.saveDecGuideMode)));
